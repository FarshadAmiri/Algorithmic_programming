...
jobs = [[int(j) for j in i] for i in jobs]

jobs_bst = BST()

day_number = 0
ans = 0
for today_jobs in jobs:
    day_number += 1
    for job in today_jobs:
        jobs_bst.insert(job - 1 + day_number)
    while jobs_bst.getMinimum() != None:
        if jobs_bst.getMinimum() < day_number:
            jobs_bst.delete(jobs_bst.getMinimum())
        else:
            break
    if jobs_bst.getMinimum() != None:
       ans += 1
       jobs_bst.delete(jobs_bst.getMinimum())
